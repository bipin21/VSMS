...
//        $this->middleware('guest', ['except' => 'logout']);
    ...
    {
        if(Auth::user()){
            $b=DB::select('SELECT distinct(model),color,engine_number,chasis_number,reg_no,price,status,id  FROM `bikes` ');
           
            $orderid=DB::select('SELECT ordername FROM orders order by id desc limit 1 ');
//            $q=DB::table('product')->get();
        return view('bill.bikebill',array('user'=>Auth::user(),'data'=>$b,'oid'=>$orderid));
             }
        else{
